...
if (!canvas) {
  ...
  function playNextMatch(): void {
      ...
      if (elements.currentMatch) {
          elements.currentMatch.textContent = `${currentMatch.player1.name} vs ${currentMatch.player2.name}`;
      }
      startCountdown();
  }
    function resetBall(): void {
        ...
        gameState.ball.angularVelocity = 0;
    ...
    function drawBall(): void {
        if (!ctx) return;
        const size   = gameState.ball.radius * 2;
        const offset = gameState.ball.radius;
        ctx.save();
        ctx.translate(gameState.ball.x, gameState.ball.y);
        ctx.rotate(gameState.ball.angle);
  ...
  function updateBall(deltaTime: number): void {
      ...
      gameState.ball.angle += gameState.ball.angularVelocity * deltaTime;
      gameState.ball.angularVelocity *= 0.995;
  }
    ...
    function checkCollisions(): void {
        ...
        if (gameState.ball.speedX < 0 &&
            ...
            // スピンを加える
            gameState.ball.angularVelocity += (hitPosition - 0.5) * 10;
