// import emailjs from 'emailjs-com'; // ★もしemail送Z信用のライブラリを使うなら適宜 import
import './register.css';
...
function updatePageTranslations(): void {
    ...
    if (googleButton) {
        ...
        googleButton.innerHTML = `${registerWithText} <img src="/images/Google_Logo.png" alt="Google Logo">`;
    }
...
export default function register() {
    ...
    form.addEventListener('submit', async (event) => {
        ...
        // ★レジスター押した時点でメール送信（2段階認証のため）
        try {
            // await fetch('http://localhost:3000/api/send-pin', {
                ...
                body: JSON.stringify({
                    to: (document.getElementById('username') as HTMLInputElement).value
                }),
            });
        } catch(e) {
            console.error('PINメール送信失敗', e);
        }

        // ★ここから変更★ サーバーでPINを確認する
        const pin = prompt('進めるには暗証番号を入力してください');
        if (pin) {
            try {
                const verifyResponse = await fetch('/api/verify-pin', {
                    ...
                    body: JSON.stringify({
                        to: usernameField.value,
                        pin: pin
                    }),
                });
                const verifyData = await verifyResponse.json();
                if (!verifyResponse.ok || !verifyData.success) {
                    messageDiv.textContent = '暗証番号が間違っています';
                    messageDiv.classList.remove('success');
                    messageDiv.classList.add('error');
                    return; // ここで処理を中断
                }
            } catch (e) {
                console.error('PIN検証エラー', e);
                messageDiv.textContent = 'PIN検証中にエラーが発生しました';
                messageDiv.classList.remove('success');
                messageDiv.classList.add('error');
                return;
            }
    ...
    if (loginLinkContainer) {
        ...
        if (!existingLink) {
            ...
            loginLinkContainer.innerHTML = `${languageService.translate('auth.has_account', 'Vous avez déjà un compte ?')} <a href="/login" style="color: #FFD500; font-weight: bold;">${loginText}</a>`;
        }
