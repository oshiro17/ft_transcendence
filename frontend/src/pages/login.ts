...
export default function login() {
    ...
    if (loginContainer && !document.getElementById('google-button')) {
        ...
        googleButton.innerHTML = `${languageService.translate('auth.login_with', 'Login with')} <img src="/images/Google_Logo.png" alt="Google Logo">`;
    ...
    if (signupLinkContainer) {
        ...
        if (!existingLink) {
            ...
            signupLinkContainer.innerHTML = `${languageService.translate('auth.no_account', 'Don\'t have an account yet?')} <a href="/register" style="color: #FFD500; font-weight: bold;">${registerText}</a>`;
        }
